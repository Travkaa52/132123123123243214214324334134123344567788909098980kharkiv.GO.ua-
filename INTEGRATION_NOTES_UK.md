...
| Обробка апдейтів (безкоштовно, за розкладом) | `frontend/scripts/process-telegram-bot.mjs` + `.github/workflows/telegram-bot.yml` | кожні 3 год піднімає завдання, яке ~1 год тримає активне long-polling (майже наживо), забирає повідомлення, шле адміну, рахує скарги, публікує оголошення |
...
4. Скарги на затримку рахуються: якщо на **один маршрут** поскаржилось
   **≥5** різних користувачів за годину (`DELAY_REPORT_THRESHOLD` /
   ...
   оголосити затримку". Адмін також може оголосити (і зняти) затримку
   вручну в будь-яку мить, без очікування скарг користувачів і без
   жодних push-сповіщень їм — командою `/alert <номер|all> [вид] текст`
   (наприклад `/alert 27 bus текст`). Після підтвердження (кнопкою) або
   команди — запис потрапляє в
   `frontend/public/data/route-alerts.json` на 2 години
   (`DELAY_ALERT_DURATION_HOURS`) і комітиться в репозиторій.
   - `/alerts` — список усіх активних оголошень з кнопкою "🗑 Скасувати"
     біля кожного, щоб зняти оголошення достроково (без очікування,
     поки воно саме "протухне").
   - Спеціальне значення `all` замість номера маршруту — "загальне"
     оголошення: `/alert all bus текст` підсвітить УСІ автобусні
     маршрути й покаже банер на весь розділ "Автобуси", а
     `/alert all текст` (без виду транспорту) підсвітить геть увесь
     розділ "Транспорт" і всі маршрути в ньому.
5. GitHub Pages роздає цей JSON як звичайний статичний файл — фронтенд
   (`useRouteAlertsStore` + `RouteAlertBanner` / `RouteCard` / нове
   `TransportAlertsBanner` для загальних оголошень) підтягує його раз на
   хвилину і показує банер/бейдж біля конкретного маршруту, або — для
   загальних оголошень — вгорі всього розділу маршрутів. Ті самі активні
   оголошення одночасно зʼявляються в розділі "Новини транспорту" на
   головній сторінці (`TransportNewsSection`) — жодних окремих дій від
   адміна для цього не потрібно, це один і той самий фід.
...
- Додано `bot/telegram_bot.py` — Python-версія того самого протоколу для
  тих, хто хоче цілодобовий постійний процес замість чергування "година з кожних трьох" в Actions.
- **Виправлено (критично):** коміти від `telegram-bot.yml` /
  `bot/telegram_bot.py` / `telegram-notifications.yml` мали позначку
  `[skip ci]` у повідомленні. Це вбудована поведінка GitHub — такий текст
  у коміті скасовує запуск УСІХ workflow'ів на цей push, включно з
  `deploy.yml`. Тобто дані (`route-alerts.json`, `notifications.json`)
  оновлювались у репозиторії, бот у Telegram чесно відповідав "✅
  Оголошення створено", а сам задеплоєний сайт на GitHub Pages ніколи не
  перезбирався й далі роздавав старі (порожні) файли — оголошення про
  затримку фізично не могло зʼявитись у застосунку. Прибрано `[skip ci]`
  з усіх трьох місць — тепер кожен коміт з новими даними автоматично
  тригерить `deploy.yml` і сайт оновлюється протягом хвилини-двох.
- **Нове:** `scripts/process-telegram-bot.mjs` (безкоштовний Actions-варіант)
  тепер вміє `/alerts` (список активних оголошень + кнопка "🗑 Скасувати"
  біля кожного) і `/help` — раніше це вміла тільки опційна Python-версія.
  Тепер видалити оголошення про затримку достроково можна незалежно від
  того, який з двох варіантів бота використовується.
- **Нове:** `/alert all [вид] текст` — "загальне" оголошення, не привʼязане
  до конкретного маршруту. Підсвічує банером увесь відповідний розділ
  маршрутів (`TransportAlertsBanner`) і всі картки маршрутів у ньому, а не
  лише один маршрут.
- **Нове:** розділ "Новини транспорту" на головній сторінці
  (`TransportNewsSection`) більше не статична демо-картка — це живий фід
  активних оголошень про затримку з `route-alerts.json` (той самий, що й
  банер біля маршруту), з фолбеком на останню новину з офіційного каналу,
  якщо активних затримок немає.
...
- Якщо працює лише `telegram-bot.yml` (без `bot/telegram_bot.py`) —
  ...
  cron у GitHub Actions — раз на 3 год, з активним чергуванням ~1 год усередині кожного запуску).
