...
**Користувачі** (приватний чат):
- `/start` — привітання + показує ваш chat_id
- `/help` — коротка довідка
- будь-який текст — йде в підтримку, адмін відповідає звичайним Reply
- кнопка «⚠️ Затримка» в застосунку — шле структуровану скаргу

**Адміни** (`ADMIN_CHAT_IDS`):
- `/alert <номер> [bus|tram|trolleybus|metro] <текст>` — оголосити затримку вручну, з кнопкою «🗑 Скасувати достроково»
- `/alerts` — список усіх активних оголошень, кожне з кнопкою скасування в 1 тап
- `/stats` — короткий дашборд: активні оголошення, скарги за останнє вікно, топ-5 маршрутів
- Inline-кнопка під авто-запитом «X користувачів поскаржились» — підтвердити оголошення в 1 тап (і одразу скасувати, якщо передумали)

Команди самі з'являються в меню "/" в Telegram (`setMyCommands` викликається при старті) — адмінам показується розширений список, звичайним користувачам лише `/start` і `/help`.

## Надійність

- Усі файли стану пишуться **атомарно** (tmp-файл + `os.replace`) — SIGKILL
  чи збій мережі посеред запису не пошкодить дані.
- Виклики Telegram API мають **ретраї з експоненційним бекофом**, включно з
  автоматичною паузою на `retry_after`, якщо Telegram повернув 429.
- Проста **антиспам-логіка**: один користувач не може накрутити поріг
  оголошення затримки чи завалити адмінів, надсилаючи скарги/звернення
  частіше, ніж `USER_RATE_LIMIT_SECONDS` (за замовчуванням 20с).
- `SIGINT`/`SIGTERM` зупиняють цикл акуратно — без обриву посеред запису
  файлів (`systemctl stop`/`restart` безпечні).
- Структуровані логи (`LOG_LEVEL` у `.env`) замість `print`.
